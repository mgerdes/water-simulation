...
void main () {
    ...
    float kd = 0.3 * max(dot(normalize(light_pos_1), normal), 0);
    kd += 0.3 * max(dot(normalize(light_pos_2), normal), 0);
    ...
    if (floor_texture_coord.x < 0.0 && floor_texture_coord.y < 0.0) {
        if (abs(floor_texture_coord.x) < abs(floor_texture_coord.y)) {
            ...
            floor_texture_coord = ((floor_hit_point + vec3(0.0, 3.0, 3.0)) / 6.0).zy;
        }
    ...
    vec2 sky_texture_coord = ((sky_hit_point + vec3(3.0, 3.0, 3.0)) / 6.0).xy;
